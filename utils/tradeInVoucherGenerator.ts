...
export const generateSellVoucherHTML = (
  request: TradeInRequest,
  opts: { sellMin: number; sellMax: number },
): string => {
  const customer   = escapeHtml(request.customer_name || '');
  const device     = escapeHtml(`${request.device_brand || ''} ${request.device_model || ''}`.trim());
  const grade      = escapeHtml((request.trade_in_grade || 'A').toUpperCase());
  const voucher    = escapeHtml(resolveVoucherReference(request));
  const issuedOn   = formatDate(request.created_at);
  const expiryIso  = resolveVoucherExpiryIso(request.voucher_expires_at, request.created_at);
  const validUntil = formatDate(expiryIso);
...
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>Bon de vente ${voucher} — Xeption</title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    @import url('https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@400;500;700;800&display=swap');
    body { background: #0a0a0a; color: #ffffff; font-family: 'Space Grotesk', Helvetica, Arial, sans-serif; margin: 0; padding: 0; }
    .pdf-document { width: 100%; max-width: 520px; margin: 0 auto; background: #0a0a0a; }
    .pdf-page-voucher { width: 520px; }
    .voucher { width: 100%; max-width: 520px; background: #111111; border: 1px solid #2a2a2a; position: relative; overflow: hidden; }
    .voucher::before { content: ''; display: block; height: 3px; background: linear-gradient(90deg, #f0d14a 0%, #c8a800 50%, #f0d14a 100%); }
    .header { padding: 28px 32px 24px; border-bottom: 1px solid #1e1e1e; display: flex; justify-content: space-between; align-items: flex-start; gap: 16px; }
    .brand-block { display: flex; align-items: center; gap: 12px; }
    .brand-logo { width: 44px; height: 44px; flex-shrink: 0; border-radius: 8px; }
    .brand-text { display: flex; flex-direction: column; gap: 6px; }
    .brand { font-size: 26px; font-weight: 800; letter-spacing: 4px; text-transform: uppercase; color: #ffffff; line-height: 1; }
    .badge { display: inline-flex; align-items: center; gap: 6px; margin-top: 8px; border: 1px solid #f0d14a33; background: #f0d14a0d; padding: 4px 10px; font-size: 9px; font-weight: 700; letter-spacing: 2px; text-transform: uppercase; color: #f0d14a; }
    .meta { text-align: right; font-size: 11px; color: #a0a0a0; line-height: 2; }
    .meta strong { color: #d0d0d0; font-weight: 500; }
    .body { padding: 28px 32px; display: flex; flex-direction: column; gap: 12px; }
    .field { background: #0f0f0f; border: 1px solid #1e1e1e; padding: 14px 18px; }
    .field-label { font-size: 9px; font-weight: 700; letter-spacing: 2px; text-transform: uppercase; color: #b0b0b0; margin-bottom: 6px; }
    .field-value { font-size: 17px; font-weight: 700; color: #ffffff; line-height: 1.2; }
    .field-sub { margin-top: 6px; font-size: 13px; font-weight: 500; color: #c8c8c8; font-family: ui-monospace, monospace; letter-spacing: 0.04em; }
    .field-sub-label { font-size: 8px; font-weight: 700; letter-spacing: 1.5px; text-transform: uppercase; color: #888; margin-right: 8px; }
    .amount-block { background: #0f0f0f; border: 1px solid #f0d14a22; padding: 28px 18px; text-align: center; position: relative; }
    .amount-block::before { content: ''; position: absolute; inset: 0; background: radial-gradient(ellipse at center, #f0d14a08 0%, transparent 70%); pointer-events: none; }
    .amount-label { font-size: 9px; font-weight: 700; letter-spacing: 2px; text-transform: uppercase; color: #b8b8b8; margin-bottom: 10px; }
    .amount-value { font-size: 36px; font-weight: 800; color: #f0d14a; letter-spacing: -1px; line-height: 1; }
    .amount-sub { font-size: 11px; color: #a0a0a0; margin-top: 10px; font-weight: 500; letter-spacing: 1px; text-transform: uppercase; }
    .validity { background: #0f0f0f; border: 1px solid #1e1e1e; padding: 14px 18px; display: flex; justify-content: space-between; align-items: center; }
    .validity-label { font-size: 10px; color: #a8a8a8; font-weight: 500; text-transform: uppercase; letter-spacing: 1px; }
    .validity-date { font-size: 13px; font-weight: 700; color: #ffffff; }
    .ref-bar { padding: 16px 32px; border-top: 1px solid #1a1a1a; display: flex; justify-content: space-between; align-items: center; }
    .ref-code { font-size: 13px; font-weight: 700; letter-spacing: 2px; color: #c0c0c0; font-family: monospace; }
    .ref-label { font-size: 9px; color: #a0a0a0; letter-spacing: 1px; text-transform: uppercase; }
    .footer { padding: 16px 32px 24px; border-top: 1px solid #141414; }
    .footer p { font-size: 10px; color: #909090; line-height: 1.7; }
    .corner { position: absolute; top: 0; right: 0; width: 60px; height: 60px; border-left: 1px solid #f0d14a1a; border-bottom: 1px solid #f0d14a1a; pointer-events: none; }
    @media print { @page { margin: 0; size: A5 portrait; } body { background: #0a0a0a !important; -webkit-print-color-adjust: exact; print-color-adjust: exact; padding: 0; } .pdf-document { max-width: none; } .voucher { max-width: none; border: none; } }
  </style>
</head>
<body>
  <div class="pdf-document">
  ...
  <div class="voucher">
    ...
    <div class="header">
      <div class="brand-block">
        ...
        <div class="brand-text">
          <div class="brand">XEPTION</div>
          <div class="badge">Vente Directe — Bon de vente</div>
        </div>
      </div>
      <div class="meta">
        <div><strong>Émis le</strong> ${issuedOn}</div>
    ...
    <div class="body">
      ...
      <div class="field">
        <div class="field-label">Appareil à vendre</div>
        <div class="field-value">${device} · Grade ${grade}</div>
        <div class="field-sub"><span class="field-sub-label">IMEI</span>${imei}</div>
      </div>
      <div class="amount-block">
        <div class="amount-label">Tu pourrais recevoir</div>
        <div class="amount-value">${formatFCFA(opts.sellMin)} – ${formatFCFA(opts.sellMax)}</div>
        <div class="amount-sub">Sous réserve de vérification en boutique</div>
      </div>
      <div class="validity">
        <span class="validity-label">Validité du bon (${validityDays} jours)</span>
    ...
    <div class="footer">
      <p>
        Valeur estimée sous réserve de vérification physique en boutique Xeption Network.<br />
        Présente une CNI ou passeport + la facture d'achat originale lors de ton RDV de vente.
      </p>
    </div>
  </div>
  </section>
...
export const downloadSellVoucher = async (
  request: TradeInRequest,
  opts: { sellMin: number; sellMax: number },
): Promise<void> => {
  ...
  const html = generateSellVoucherHTML(request, opts);
  ...
  try {
    ...
    addCanvasToA5Page(pdf, canvas, 0);
    pdf.save(`Xeption-BonVente-${voucherRef}.pdf`);
  } finally {
    document.body.removeChild(wrapper);
  }
};
