            ...
            <div className="flex-1 overflow-y-auto py-6 px-3 space-y-1">
                {MENU_ITEMS.map(item => (
                    <Link
                        key={item.id}
                        to={`/admin/${item.id}`}
                        className={`w-full flex items-center gap-3 px-4 py-3 rounded-md text-sm font-bold uppercase tracking-wider transition-all ${currentPath === item.id
            ...
            <div className="flex overflow-x-auto no-scrollbar py-2 px-2 gap-2">
                {MENU_ITEMS.map(item => (
                    <Link
                        key={item.id}
                        to={`/admin/${item.id}`}
                        className={`flex flex-col items-center justify-center min-w-[70px] p-2 rounded-lg transition-all ${currentPath === item.id ? 'text-xeption-gold' : 'text-gray-500'
                            }`}
                    >
                        <item.icon className={`w-5 h-5 mb-1 ${currentPath === item.id ? 'fill-current' : ''}`} />
        ...
        <div className="min-h-screen text-white font-sans selection:bg-xeption-gold selection:text-black">
            ...
            <main className="md:ml-64 p-4 md:p-8 pb-24 md:pb-8 min-h-screen relative overflow-x-hidden">
                ...
                <Routes>
                    ...
                    <Route path="guide" element={renderGuide()} />
                    <Route path="*" element={<Navigate to="dashboard" replace />} />
                </Routes>
            </main>
                            ...
                            <div className="space-y-4">
                                <div className="bg-black/40 backdrop-blur-md p-6 border border-white/10 rounded-sm">
                                    ...
                                    <div className="grid grid-cols-3 gap-3 mb-3">
                                        <input type="number" min="0" className="bg-black border border-white/10 p-3 text-white" placeholder="Prix" value={editingProduct.price} onChange={e => setEditingProduct({ ...editingProduct, price: Math.max(0, +e.target.value) })} />
                                        <input type="number" min="0" className="bg-black border border-white/10 p-3 text-white" placeholder="Stock" value={editingProduct.stock} onChange={e => setEditingProduct({ ...editingProduct, stock: Math.max(0, +e.target.value) })} />
                                        <input type="number" min="0" className="bg-black border border-white/10 p-3 text-white" placeholder="Garantie (mois)" value={editingProduct.warrantyMonths || 0} onChange={e => setEditingProduct({ ...editingProduct, warrantyMonths: Math.max(0, +e.target.value) })} />
                                    </div>
                                    <div className="mb-3">
