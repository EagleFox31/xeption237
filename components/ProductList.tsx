    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
            ...
            <button
              ...
              className={`px-5 py-2 rounded-none border border-transparent font-tech font-bold uppercase tracking-wider text-sm transition-all clip-path-slant backdrop-blur-md ${
                filter === cat 
                  ...
                  : 'bg-black/40 text-gray-400 border-white/10 hover:border-white hover:text-white hover:bg-black/60'
              }`}
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {filteredProducts.map((product) => (
          <div 
            key={product.id}
            className="group relative bg-black/40 backdrop-blur-xl border border-white/5 hover:border-xeption-gold/40 transition-all duration-300 flex flex-col overflow-hidden hover:shadow-[0_0_30px_rgba(0,0,0,0.5)] hover:-translate-y-1 cursor-pointer"
            onClick={() => onProductClick && onProductClick(product)}
          >
            {/* Promo Tag */}
            {product.isPromo && (
              <div className="absolute top-0 right-0 z-20">
                 <div className="bg-xeption-red text-white text-[10px] font-bold px-3 py-1 font-tech uppercase tracking-widest shadow-[0_0_10px_#ff0033]">
                    Promo
                 </div>
              </div>
            )}
            ...
            <div className="aspect-[4/3] bg-black/50 relative overflow-hidden">
              <img 
                src={product.image} 
                alt={product.name} 
                className="w-full h-full object-cover object-center group-hover:scale-105 group-hover:opacity-80 transition-all duration-500"
              />
              {/* Overlay Gradient */}
              ...
              <div className="absolute bottom-4 right-4 translate-y-10 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300 z-30">
                 <button 
                  ...
                  className="bg-xeption-gold text-black p-3 hover:bg-white transition-colors shadow-lg"
                >
            ...
            <div className="p-6 flex-1 flex flex-col relative">
              <div className="mb-2">
                <h3 className="text-xl font-bold text-white font-tech uppercase tracking-wide group-hover:text-xeption-gold transition-colors truncate">
                  {product.name}
                </h3>
              </div>
            ...
            <div className="absolute bottom-0 left-0 w-full h-[2px] bg-gradient-to-r from-transparent via-xeption-gold to-transparent opacity-0 group-hover:opacity-100 transition-opacity"></div>
          </div>
        ))}
      </div>
    </div>
