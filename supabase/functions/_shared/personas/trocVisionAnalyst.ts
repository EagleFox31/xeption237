...
export const buildTrocVisionAnalystPrompt = (input: TrocVisionAnalystInput): string => {
  ...
  return [
    ...
    '- Contexte réel : le smartphone peut être posé sur un bureau, une table, à côté d’un PC portable ou tenu en main. La présence d’un laptop ou d’objets en arrière-plan est normale, concentre-toi sur le smartphone.',
    ...
    '- Une photo est NON CONFORME si et seulement si elle correspond à un de ces cas :',
    '    • capture d’écran logicielle (screenshot, interface logicielle d’ordinateur, page web) → reason = "screenshot"',
    '    • objet qui n’est pas un téléphone (montre seule, bracelet connecté, tablette, écouteurs, PC seul sans smartphone, console, personne, plat, document, animal) → reason = "not_a_device"',
    '    • image de catalogue, rendu 3D, photo marketing, dessin, illustration commerciale → reason = "rendered_or_marketing"',
    '    • photo totalement noire ou si floue que l’appareil n’est PAS identifiable même partiellement → reason = "severely_unreadable"',
    ...
    '- Une photo prise sous éclairage ordinaire, avec reflets ou environnement domestique MAIS où l’appareil reste identifiable EST conforme. Sois tolérant tant que l’on voit le smartphone.',
    '',
    ...
    '- Les smartphones récents partagent un format commun (dos vitré ou mat, bloc photo multi-capteurs, écran poinçonné). Si les photos montrent un smartphone moderne cohérent sans logo contradictoire, considère-le compatible (mets decision = "match" ou "uncertain" si l’identification exacte n’est pas certaine à 100%).',
    '- Ne mets decision = "mismatch" QUE si une marque TOTALEMENT OPPOSÉE est incontestablement visible (ex: logo Apple indiscutable alors que la déclaration est Xiaomi ou Samsung, ou ancien téléphone à touches). Ne JAMAIS mettre "mismatch" sur un simple doute.',
    '- "fraud_suspected" est réservé aux cas extrêmes : preuves de montage ou trucage flagrant.',
    '- Si l’appareil semble cohérent mais qu’une partie de l’état reste incertaine, mets decision = "uncertain" et confidence moyenne.',
