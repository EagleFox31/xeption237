...
const InventoryTab: React.FC<InventoryTabProps> = ({ products, onEditProduct, onDeleteProduct, onCreateProduct }) => {
  return (
                    ...
                    {products.map(product => (
                        <tr key={product.id} className="hover:bg-white/5 transition-colors">
                            <td className="px-6 py-4 flex items-center gap-4">
                                <img src={product.image} className="w-10 h-10 object-contain bg-black rounded p-1" alt={product.name}/>
                                <div><span className="font-bold text-white block">{product.name}</span><span className="text-xs text-gray-500 capitalize">{product.category}</span></div>
                            </td>
                            ...
                            <td className="px-6 py-4 text-right">
                                <button onClick={() => onEditProduct(product)} className="p-2 text-xeption-gold hover:bg-white/10 rounded"><Edit className="w-4 h-4" /></button>
