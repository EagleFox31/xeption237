...
import { ShoppingCart, Menu, X, Search, Lock, ArrowRight, Tag } from 'lucide-react';
            ...
            {searchQuery && (results.products.length > 0 || results.categories.length > 0) && (
              <div className="absolute top-full left-0 right-0 mt-2 bg-[#09090b]/95 backdrop-blur-xl border border-white/10 rounded-xl shadow-[0_10px_40px_rgba(0,0,0,0.5)] overflow-hidden z-50 animate-in fade-in slide-in-from-top-2">
                ...
                {results.categories.length > 0 && (
                  <div className="p-2 border-b border-white/5 bg-white/5">
                    ...
                    {results.categories.map(cat => (
                      <button
                        key={cat}
                        onClick={() => navigate('/shop')} // Ideally filter shop by cat
                ...
                <div className="p-2 bg-black/40 text-center border-t border-white/5">
                  <button onClick={() => navigate('/shop')} className="text-xs text-gray-400 hover:text-white transition-colors">
          ...
          <nav className="hidden md:flex space-x-1">
            {navItems.map((item) => (
              <button
                ...
                className={`relative px-4 py-2 rounded text-sm font-bold tracking-wide transition-all duration-300 font-tech uppercase ${currentPath === item.id
          ...
          <div className="px-4 pt-4 pb-6 space-y-2">
            {navItems.map((item) => (
              <button
                key={item.id}
                onClick={() => handleNav(item.id)}
                className={`block w-full text-left px-4 py-3 rounded-lg text-lg font-tech uppercase font-bold tracking-wider ${currentPath === item.id
