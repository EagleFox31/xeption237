...
const getCategoryHeroCopy = (slug: string, name: string) => {
  if (slug === 'phones' || slug === 'smartphones') {
    return {
      ...
      mascotMsg: 'Les meilleurs téléphones certifiés ! ✨'
    };
  }
  if (slug === 'tablets' || slug === 'tablettes') {
    return {
      ...
      mascotMsg: 'iPad & tablettes au top ! ✨'
    };
  }
  if (slug === 'laptops' || slug === 'ordinateurs') {
    return {
      ...
      mascotMsg: 'MacBook & PC pour bosser et performer ! 💻✨'
    };
  }
  if (slug === 'gaming' || slug === 'consoles') {
    return {
      ...
      mascotMsg: 'PS5 et setups prêts à gamer ! 🎮✨'
    };
  }
  if (slug === 'accessories') {
    return {
      ...
      mascotMsg: 'Coques, écouteurs et montres connectées ! ✨'
    };
  }
  return {
    title: name,
    subtitle: `Découvre notre sélection ${name.toLowerCase()} chez Xeption Network.`,
    mascotMsg: 'Trouve les meilleures pépites tech du 237 ! 🛍️✨'
  };
};
...
const ShopHero: React.FC<ShopHeroProps> = ({
  ...
  const defaultCopy = {
    ...
    mascotMsg: 'Trouve tes pépites tech au meilleur prix ! 🛍️✨'
  };
  ...
  return (
    <section className="relative border-b border-white/10 bg-black/70 backdrop-blur-sm shadow-lg overflow-hidden">
      ...
      <div className="relative max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 pt-3 pb-3 md:pt-4 md:pb-4">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-5 md:gap-8">
          <div className="min-w-0 flex-1">
            <p className="text-[10px] font-tech uppercase tracking-[0.25em] text-xeption-gold mb-1">
            ...
            <p className="text-xs md:text-sm text-gray-400 mt-1 max-w-2xl leading-relaxed">{copy.subtitle}</p>
            {searchQuery.trim() ? (
              <div className="mt-2">
          ...
          <div className="flex items-center gap-6 self-start md:self-center shrink-0">
            {/* Mascotte Xepti Shopping 3D avec Sac de Luxe */}
            <div className="hidden sm:flex items-center justify-center -my-3">
              <ChameleoMascot 
                size="sm"
                pose="shopping"
                state="idle"
                message={copy.mascotMsg}
              />
            </div>
