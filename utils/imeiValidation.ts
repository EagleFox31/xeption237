...
export const isValidImei = (imei: string): boolean => {
  const normalized = sanitizeImei(imei);
  return /^\d{15}$/.test(normalized) && luhnCheck(normalized) && !isTrivialTestImei(normalized);
};

export const describeImeiInputError = (imei: string): string | null => {
  const normalized = sanitizeImei(imei);
  if (!normalized) return null;
  if (!/^\d{15}$/.test(normalized)) return null;
  if (isTrivialTestImei(normalized)) {
    return 'Ça ressemble à un numéro de test — compose *#06# sur ton téléphone pour obtenir le vrai IMEI.';
  }
  if (!luhnCheck(normalized)) {
    return 'Ce numéro ne passe pas le contrôle IMEI — revérifie les 15 chiffres affichés par *#06#.';
  }
  return null;
};
