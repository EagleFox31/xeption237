...
const CategoriesTab: React.FC<CategoriesTabProps> = ({
  ...
  return (
    <div className={embedded ? '' : 'animate-in fade-in slide-in-from-bottom-5'}>
        {!embedded && (
          <h2 className="text-3xl font-tech font-bold uppercase text-white mb-6">
            Gestion des Types (Types de Produits)
          </h2>
        )}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="md:col-span-2 bg-black/40 backdrop-blur-md border border-white/10 rounded-sm overflow-hidden">
                <table className="w-full text-left">
                    <thead className="bg-black/40 text-white/80 text-xs uppercase font-bold">
                    ...
                    <tbody className="divide-y divide-white/5">
                        {categories.map(cat => (
                            <tr key={cat.id} className="hover:bg-white/5 text-sm">
                                ...
                                <td className="px-6 py-4 text-white/70 font-mono text-xs">{cat.slug}</td>
